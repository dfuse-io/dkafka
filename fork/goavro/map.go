...
func makeMapCodec(converters map[string]ConvertBuild, st map[string]*Codec, namespace string, schemaMap map[string]interface{}, cb *codecBuilder) (*Codec, error) {
	// map type must have values
	valueSchema, ok := schemaMap["values"]
	if !ok {
		return nil, errors.New("Map ought to have values key")
	}
	valueCodec, err := buildCodec(converters, st, namespace, valueSchema, cb)
	if err != nil {
